[**@urban-toolkit/autk-plot**](../index.md)

***

[@urban-toolkit/autk-plot](../globals.md) / PlotEventRecord

# Type Alias: PlotEventRecord

> **PlotEventRecord** = `Record`\<[`PlotEvent`](../enumerations/PlotEvent.md), [`PlotEventData`](PlotEventData.md)\>

Defined in: [types-events.ts:40](https://github.com/urban-toolkit/autark/blob/2086406f6ed56aea8faab9b6b840f71fa86be019/autk-plot/src/types-events.ts#L40)

Event map consumed by the typed plot event emitter.

Each plot interaction event resolves to a `PlotEventData` payload.
